...
#define TOKEN_CONTRACT N(enu.token)
...
class playerone: public enumivo::contract {
public:
    ...
    void enu_token_transfer(account_name from, account_name to, asset quantity, string memo){
        require_auth(from);
        if (from == _self || to != _self) {
            return;
        }
        enumivo_assert(quantity.is_valid(), "invalid token transfer");
        enumivo_assert(quantity.symbol == CORE_SYMBOL, "unexpected asset symbol input");
    ...
    void game_token_transfer(account_name from, account_name to, asset quantity, string memo){
        ...
        enumivo_assert(quantity.is_valid(), "invalid token transfer");
        enumivo_assert(quantity.symbol == GAME_SYMBOL, "unexpected asset symbol input");
    ...
    void transfer(account_name from, account_name to, asset quantity, string memo){
        ...
        if(quantity.symbol == CORE_SYMBOL){
            if(quantity.amount == 1ll){
                ...
                if( now() < _GAME_PRESALE_TIME ){
                    ...
                    if(user_itr == userinfo.end() || quantity.amount > user_itr->refer * _UNIT + user_itr->invitation * _UNIT + _MAX_IN_PRESALE){
                        enumivo_assert( quantity.amount >= _UNIT && quantity.amount <= _MAX_IN_PRESALE, "预售份额不足，存入EOS获得等量不受限的份额（不能退回）或者单次购买 1 - 10 EOS");
            ...
            enumivo_assert( now() >= _GAME_PRESALE_TIME, "预售阶段不能够销毁、抵押、出售CGT");
            if(memo == "burn"){
                burn(from, quantity);
            } else if(memo == "stake"){
            ...
            enumivo_assert(false, "不要转入其他代币资产");
        }
    };
    ...
    void buy(account_name account, asset quantity, string memo){
        ...
        if(user_itr == userinfo.end()){
            ...
            enumivo_assert( now_action >= user_itr->last_action + _ACTION_COOL_DOWN, "操作太频繁，需要时间冷却");
        ...
        enumivo_assert(transfer_token <= game_itr->balance, "insufficient token balance");
        enumivo_assert(refund_eos == remain_eos && refund_eos >= asset(0, CORE_SYMBOL) && refund_eos <= quant_after_fee, "invalid eos refund");
        enumivo_assert(deposited_eos >= asset(0, CORE_SYMBOL) && insured_eos >= asset(0, CORE_SYMBOL), "eos deposit or insure must be positive");
        enumivo_assert(transfer_token >= asset(0, GAME_SYMBOL) && issue_token >= asset(0, GAME_SYMBOL), "transfer and issue token should not be negetive");
        enumivo_assert(exchanged_eos + issued_eos == deposited_eos + insured_eos && quant_after_fee - remain_eos == deposited_eos + insured_eos, "eos not equal");
        enumivo_assert(transfer_token + issue_token >= asset(_UNIT, GAME_SYMBOL) && transfer_token + issue_token <= asset(10000 * _UNIT, GAME_SYMBOL), "transfer and issue token must in range 1 - 10000");
    ...
    void sell(account_name account, asset quantity, string memo){
        enumivo_assert(quantity.amount >= _UNIT && quantity.amount <= 10000 * _UNIT, "卖出CGT区间为 1 - 10000 CGT");
        ...
        for(int64_t i = 0; i < times; i++){
            ...
            if(reserve_balance < asset(0, CORE_SYMBOL)){
                enumivo_assert(false, "insufficient reserve eos");
            }
        ...
        if(user_itr == userinfo.end()){
            ...
            enumivo_assert( now() >= user_itr->last_action + _ACTION_COOL_DOWN, "操作太频繁，需要时间冷却");
            userinfo.modify(user_itr, account, [&](auto& u) {
                u.last_action = now();
            });
        }
    ...
    void burn(account_name account, asset quantity){
        enumivo_assert(quantity.amount >= _UNIT && quantity.amount <= 10000 * _UNIT, "销毁CGT的区间为 1 - 10000 CGT");
    ...
    void stake(account_name account, asset quantity){
        ...
        if(account == game_itr->player_one){
            ...
            enumivo_assert(false, "需要抵押更多的CGT才能超越当前的头号");
        }
    }

    void unstake(account_name account){
        auto game_itr = _game.begin();
        enumivo_assert(game_itr->player_one == account, "can only unstake player one's token");
    ...
    void claim_fee(account_name account){
        ...
        if(user_itr->reward > asset(_UNIT, CORE_SYMBOL)){
            ...
            //推荐人奖励金池与主奖金池隔离
            enumivo_assert(game_itr->fee >= asset(0, CORE_SYMBOL), "shit happens");
            action(
                permission_level{_self, N(active)},
                TOKEN_CONTRACT, N(transfer),
    ...
    real_type _crr(asset circulation) {
        enumivo_assert(circulation.amount >= 0, "shit happens");
        real_type A(_A);
        real_type B(_B);
        real_type L(_L);
        real_type D(_D);
        ...
        enumivo_assert(R >= B && R <= B + A, "shit happens");
        return R / H;
    }
    ...
    typedef enumivo::multi_index<N(game), game> game_table;
    game_table _game;
    ...
    struct user{
        ...
        uint64_t primary_key() const { return gameid; }
        ENULIB_SERIALIZE(user, (gameid)(name)(parent)(reward)(last_action)(refer)(invitation)(discount))
    };
    typedef enumivo::multi_index<N(userinfo), user> user_table;
    // user_table userinfo;
    ...
    struct refer{
        ...
        ENULIB_SERIALIZE(refer, (name))
    };
    typedef enumivo::multi_index<N(refers), refer> refer_table;
    refer_table refers;
    ...
    struct invitation{
        ...
        ENULIB_SERIALIZE(invitation, (to)(from))
    };
    typedef enumivo::multi_index<N(invitations), invitation> invitation_table;
    invitation_table invitations;
};

#define ENU_ABI_EX( TYPE, MEMBERS ) \
extern "C" { \
    void apply( uint64_t receiver, uint64_t code, uint64_t action ) { \
        auto self = receiver; \
        if(action == N(onerror)) { \
            /* onerror is only valid if it is for the "enu" code account and authorized by "enu"'s "active permission */ \
            enumivo_assert(code == N(enumivo), "onerror action's are only valid from the \"enu\" system account"); \
        } \
        if(action == N(transfer)) { \
            auto _action = N(onerror); \
            if(code == TOKEN_CONTRACT) { \
                _action = N(enu_token_transfer); \
...
ENU_ABI_EX(playerone, (enu_token_transfer)(game_token_transfer))